...
function App() {
  ...
  const allAudios = [[audio1State,setAudio1State ,audio1,"Audio 1"],
  [audio2State,setAudio2State ,audio2,"Audio 2"],[audio3State,setAudio3State ,audio3,"Audio 3"],
  [audio4State,setAudio4State ,audio4,"Audio 4"],[audio5State,setAudio5State ,audio5,"Audio 5"],
  [audio6State,setAudio6State ,audio6,"Audio 6"],[audio7State,setAudio7State ,audio7,"Audio 7"],
  [audio8State,setAudio8State ,audio8,"Audio 8"],[audio9State,setAudio9State ,audio9,"Audio 9"]
];

  const arrayOfAudios = [audio1,audio2,audio3,audio4,audio5,audio6,audio7,audio8,audio9];
 ...
 function playAudios(){
    ...
    // setTimeout(function() {}, 8); //delay 8 seconds
    // playAudios()
    sleep(0).then(() => {
      playAudios()
  });
...
{/* ------------------------------------------------------------------- */}
  ...
  return (
    <div className="App">
      ...
      {/* -------------------------------STOP-------------------------------------- */}
      {/* <div id="stop">
         <button id="playStopBtn" onClick={()=> stopAudios()
        }>
       <img id="icon"  src = {iconStop}/> 
...
<div id="stop">
       ...
       <button id="audio-button"  onClick={
         ()=>
         getAudio(sound[0],sound[1], sound[2])
      }><img  id="icon" src = {iconBtn} />{sound[3]}</button>
       </div>
     )}
